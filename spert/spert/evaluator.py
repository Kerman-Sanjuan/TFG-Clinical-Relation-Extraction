...
class Evaluator:
    ...
    def store_examples(self):
        if jinja2 is None:
            warnings.warn(
                "Examples cannot be stored since Jinja2 is not installed.")
            return
    ...
    def _score(self, gt: List[List[Tuple]], pred: List[List[Tuple]], print_results: bool = True):
        assert len(gt) == len(pred)
        ...
        print("GT_flat", pred_flat)
        return metrics

    def _compute_metrics(self, gt_all, pred_all, types, print_results: bool = True):
        labels = [t.index for t in types]
        per_type = prfs(gt_all, pred_all, labels=labels,
                        average=None, zero_division=0)
        micro = prfs(gt_all, pred_all, labels=labels,
                     average='micro', zero_division=0)[:-1]
        macro = prfs(gt_all, pred_all, labels=labels,
                     average='macro', zero_division=0)[:-1]
        total_support = sum(per_type[-1])
        confussion_matrix_value = confusion_matrix(gt_all, pred_all, labels=labels)
        print(confusion_matrix)
        with open("randomfile.txt", "a") as o:
            o.write(str(confussion_matrix_value))
        if print_results:
    ...
    def _convert_example(self, doc: Document, gt: List[Tuple], pred: List[Tuple],
        ...
        gt, pred = self._convert_by_setting(
            [gt], [pred], include_entity_types=include_entity_types, include_score=True)
        gt, pred = gt[0], pred[0]
